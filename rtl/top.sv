...
*/
  ...
  // assign data_left_lo = data_left_li;
   ...
   wire [11:0] sinOut;
  //  assign data_left_lo = ready_li? sinOut : 12'hXXX;

   sinusoid sincity (.clk_i(clk_o),.reset_i(reset_r),.sinOut(sinOut));
   //.valid_i(1'b1),.ready_o(ready_lo),.valid_o(valid_lo),.ready_i(ready_li),
   fifo_1r1w #(.depth_log2_p(5),.width_p(12)) fifo (
    .clk_i(clk_o),
   .reset_i(reset_r),
   .data_i(sinOut),
   .valid_i(1'b1),
